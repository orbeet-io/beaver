...
var (
	defaultFileMod os.FileMode = 0o600
	defaultDirMod  os.FileMode = 0o700
	yttCmd                     = "ytt"
	helmCmd                    = "helm"
	kubectlCmd                 = "kubectl"
)
...
func (r *Runner) kustomize(tmpDir string, input *os.File) (*os.File, error) {
	...
	_, err = f.WriteString(fmt.Sprintf("resources: [%s]", filepath.Base(input.Name())))
	if err != nil {
